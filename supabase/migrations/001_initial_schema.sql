...
CREATE TYPE company_size AS ENUM ('startup', 'mid', 'enterprise', 'any');
...
CREATE TABLE public.preferences (
    ...
    work_type work_type DEFAULT 'flexible',
    company_size_preference company_size DEFAULT 'any',
...
CREATE TABLE public.jobs (
    ...
    job_embedding vector(1536), -- For semantic search
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE INDEX ON public.jobs USING ivfflat (job_embedding vector_cosine_ops) WITH (lists = 100);
CREATE INDEX ON public.qualifications USING ivfflat (profile_embedding vector_cosine_ops) WITH (lists = 100);
